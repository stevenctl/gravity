...
public class Level6 extends PlayState {
	...
	long ticks = 0;
	public Level6() {
		super("level6.tmx", Angles.DOWN, 6 * 9.8f);
	...
	public void init(){
		super.init();
		p = new Player(this,3f, 96.02f);
		this.setGravity(3 * (float)Math.PI / 2f);
		
	   rayHandler.setAmbientLight(0.4f, 0.4f, 0.6f, 0.78f);
		
		
		Door door = new Door(this,70,28.02f);
		door.setDestination(State.Menu);
		door.setGravityDir(Directions.Down);
		entities.add(door);
	}
